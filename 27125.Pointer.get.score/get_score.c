...
{
	int numStu2; // 입력받는 학생 수
	int numCourse2; // 입력받는 과목 수
	...
	else if (  (numStu2 < -2 || numStu2 > 100) // 학생수= 0이하 or 100초과
			   || (numCourse2 < -2 || numCourse2 > 10)  ) // 과목수=0이하 or 10초과
	{
	...
	// 학생수=숫자가 아닌 문자가 되면, 0을 돌려줌으로써 종료
	else if (numStu2 == 0 || numCourse2 == 0 ) 
	{
		return 0;
	}
	else 
